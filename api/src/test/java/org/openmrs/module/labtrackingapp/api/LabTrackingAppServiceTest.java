...
public class LabTrackingAppServiceTest extends BaseModuleContextSensitiveTest {
	...
	private static final int TOTAL_URGENT_REVIEW_ORDERS = 1;
	...
	public void getActiveOrders_shouldGetBySuspectedCancer() throws Exception {
		...
		int status = LabTrackingConstants.LabTrackingOrderStatus.ALL.getId();
		List<Order> list = service.getActiveOrders(startDate, endDate, null, null, status, true, false, 0);
		assertEquals(TOTAL_SUSPECTED_CANCER_ORDERS, list.size());
	}

	@Test
	@Verifies(value = "should get orders that have corresponding encounter with urgentReview obs", method = "getActiveOrders()")
	public void getActiveOrders_shouldGetByUrgentReview() throws Exception {
		Date testDate = getTestEncounterDate();
		long startDate = 0;
		long endDate = 0;

		int status = LabTrackingConstants.LabTrackingOrderStatus.ALL.getId();
		List<Order> list = service.getActiveOrders(startDate, endDate, null, null, status, false, true, 0);
		assertEquals(TOTAL_URGENT_REVIEW_ORDERS, list.size());
	}
